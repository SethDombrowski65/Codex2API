...
func (h *OpenAIGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if err := json.Unmarshal(body, &chatReq); err != nil {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "Invalid request format")
		return
	}
	...
	responsesReq, err := service.ConvertChatCompletionToResponses(&chatReq)
	if err != nil {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "Failed to convert request: "+err.Error())
		return
	}
	...
	c.Request.Body = io.NopCloser(strings.NewReader(string(responsesBody)))
	defer func() {
		c.Request.Body = originalBody
	}()

	c.Set("_chat_completion_mode", true)

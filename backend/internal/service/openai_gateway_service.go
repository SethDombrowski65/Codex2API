	...
	for {
		...
		case ev, ok := <-events:
			...
			if openaiSSEDataRe.MatchString(line) {
				...
				// Check if in ChatCompletion mode and convert format
				isChatCompletionMode := false
				if c != nil {
					if val, exists := c.Get("_chat_completion_mode"); exists {
						isChatCompletionMode, _ = val.(bool)
					}
				}

				if isChatCompletionMode && data != "" && data != "[DONE]" {
					convertedLine, err := ConvertResponsesStreamToChatCompletionStream(line)
					if err == nil && convertedLine != "" {
						line = convertedLine
					}
				}
...
func (s *OpenAIGatewayService) handleNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string) (*OpenAIUsage, error) {
	...
	if isChatCompletionMode {
		var responsesResp map[string]any
		if err := json.Unmarshal(body, &responsesResp); err == nil {
			chatResp, err := ConvertResponsesToChatCompletion(responsesResp)
			if err == nil {
				if chatBody, err := json.Marshal(chatResp); err == nil {
					body = chatBody
				}
			}
		}
